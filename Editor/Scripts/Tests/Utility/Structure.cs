...
internal static partial class TestsUtility
{
    ...
    private static void CheckTopLevelDirectories(ref bool isValid, string path, PackageKey key)
    {
        ...
        ValidateDirectories(ref localIsValid, path, new[] {"Editor"}, new[] {"Runtime", ".git"}, out var required, out var tolerated);
    ...
    private static void CheckEditorDirectory(ref bool isValid, string path, PackageKey key)
    {
        ...
        ValidateDirectories(ref localIsValid, path, new[]{"Resources", "Scripts"}, null, out var requiredDirectories, out var _);

        if (requiredDirectories[0])
            ValidateResourceFolder(ref localIsValid, path, key);

        if (requiredDirectories[1])
        {
            ValidateNamingOfFilesInFolderAndSubFolders(ref localIsValid, Path.Combine(path, "Scripts"));
            ValidateFiles(ref localIsValid, Path.Combine(path, "Scripts"), new[]
            {
                "Constants.cs", "ContextMenu.cs", "DisplayContent.cs", "SaveValues.cs"
            }, null, out var _, out var _, true);
        }

        var assemblyName = GetAssemblyName(PackageKey.Undefined, "editor");

        ValidateFiles(ref localIsValid, path, new[] {assemblyName}, new[] {$"{assemblyName}.meta", "Resources.meta", "Scripts.meta"}, out var required, out var _);
    ...
    private static void ValidateResourceFolder(ref bool isValid, string path, PackageKey key)
    {
        ...
        ValidateDirectories(ref isValid, path, new[] {"User Interface"}, new []{"Images"}, out required, out var tolerated);
    ...
    private static void CheckRuntimeDirectory(ref bool isValid, string path, PackageKey key)
    {
        ...
        ValidateFiles(ref localIsValid, path, new[] {assemblyName}, new[] {$"{assemblyName}.meta", "Scripts.meta"}, out var required, out var _);
