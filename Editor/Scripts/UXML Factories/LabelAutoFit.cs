...
namespace Editor.Scripts.UXML_Factories
{
    public class LabelAutoFit : Label
    {
        ...
        private void UpdateFontSize()
        {
            ...
            {
                Vector2 textSize = MeasureTextSize(text, float.MaxValue, MeasureMode.AtMost, float.MaxValue,
                    MeasureMode.AtMost);
                float
                    fontSize = Mathf.Max(style.fontSize.value.value,
                        1); // Unity can return a font size of 0 which would break the auto fit // Should probably wait till the end of frame to get the real font size
